...
abstract class BasesfGuardUser extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array botSession[] Collection to store aggregation of botSession objects.
	 */
	protected $collbotSessions;

	/**
	 * @var        Criteria The criteria used to select the current contents of collbotSessions.
	 */
	private $lastbotSessionCriteria = null;

	/**
	 * @var        array botconfig[] Collection to store aggregation of botconfig objects.
	 */
	protected $collbotconfigs;

	/**
	 * @var        Criteria The criteria used to select the current contents of collbotconfigs.
	 */
	private $lastbotconfigCriteria = null;
	...
	/**
	 * @var        array balanceOperation[] Collection to store aggregation of balanceOperation objects.
	 */
	protected $collbalanceOperations;

	/**
	 * @var        Criteria The criteria used to select the current contents of collbalanceOperations.
	 */
	private $lastbalanceOperationCriteria = null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collbotSessions = null;
			$this->lastbotSessionCriteria = null;

			$this->collbotconfigs = null;
			$this->lastbotconfigCriteria = null;
			...
			$this->collbalanceOperations = null;
			$this->lastbalanceOperationCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collbotSessions !== null) {
				foreach ($this->collbotSessions as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
			...
			if ($this->collbotconfigs !== null) {
				foreach ($this->collbotconfigs as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
			...
			if ($this->collbalanceOperations !== null) {
				foreach ($this->collbalanceOperations as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getbotSessions() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addbotSession($relObj->copy($deepCopy));
				}
			}

			foreach ($this->getbotconfigs() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addbotconfig($relObj->copy($deepCopy));
				}
			}
			...
			foreach ($this->getbalanceOperations() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addbalanceOperation($relObj->copy($deepCopy));
				}
			}
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related messagess from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	...
	/**
	 * Clears out the collbotSessions collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 ...
	 * @see        addbotSessions()
	 */
	public function clearbotSessions()
	{
		$this->collbotSessions = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collbotSessions collection (array).
	 *
	 * By default this just sets the collbotSessions collection to an empty array (like clearcollbotSessions());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initbotSessions()
	{
		$this->collbotSessions = array();
	}

	/**
	 * Gets an array of botSession objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related botSessions from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array botSession[]
	 * @throws     PropelException
	 */
	public function getbotSessions($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collbotSessions === null) {
			if ($this->isNew()) {
			   $this->collbotSessions = array();
				...
				$criteria->add(botSessionPeer::USER_ID, $this->id);

				botSessionPeer::addSelectColumns($criteria);
				$this->collbotSessions = botSessionPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				botSessionPeer::addSelectColumns($criteria);
				if (!isset($this->lastbotSessionCriteria) || !$this->lastbotSessionCriteria->equals($criteria)) {
					$this->collbotSessions = botSessionPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastbotSessionCriteria = $criteria;
		return $this->collbotSessions;
	}

	/**
	 * Returns the number of related botSession objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related botSession objects.
	 * @throws     PropelException
	 */
	public function countbotSessions(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collbotSessions === null) {
			if ($this->isNew()) {
				...
				$count = botSessionPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastbotSessionCriteria) || !$this->lastbotSessionCriteria->equals($criteria)) {
					...
					$count = count($this->collbotSessions);
				}
			} else {
				$count = count($this->collbotSessions);
			}
	...
	/**
	 * Method called to associate a botSession object to this object
	 * through the botSession foreign key attribute.
	 *
	 * @param      botSession $l botSession
	 * @return     void
	 * @throws     PropelException
	 */
	public function addbotSession(botSession $l)
	{
		if ($this->collbotSessions === null) {
			$this->initbotSessions();
		}
		if (!in_array($l, $this->collbotSessions, true)) { // only add it if the **same** object is not already associated
			array_push($this->collbotSessions, $l);
			$l->setsfGuardUser($this);
		}
	}

	/**
	 * Clears out the collbotconfigs collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 ...
	 * @see        addbotconfigs()
	 */
	public function clearbotconfigs()
	{
		$this->collbotconfigs = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collbotconfigs collection (array).
	 *
	 * By default this just sets the collbotconfigs collection to an empty array (like clearcollbotconfigs());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initbotconfigs()
	{
		$this->collbotconfigs = array();
	}

	/**
	 * Gets an array of botconfig objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related botconfigs from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array botconfig[]
	 * @throws     PropelException
	 */
	public function getbotconfigs($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collbotconfigs === null) {
			if ($this->isNew()) {
			   $this->collbotconfigs = array();
				...
				$criteria->add(botconfigPeer::USER_ID, $this->id);

				botconfigPeer::addSelectColumns($criteria);
				$this->collbotconfigs = botconfigPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				botconfigPeer::addSelectColumns($criteria);
				if (!isset($this->lastbotconfigCriteria) || !$this->lastbotconfigCriteria->equals($criteria)) {
					$this->collbotconfigs = botconfigPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastbotconfigCriteria = $criteria;
		return $this->collbotconfigs;
	}

	/**
	 * Returns the number of related botconfig objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related botconfig objects.
	 * @throws     PropelException
	 */
	public function countbotconfigs(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collbotconfigs === null) {
			if ($this->isNew()) {
				...
				$count = botconfigPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastbotconfigCriteria) || !$this->lastbotconfigCriteria->equals($criteria)) {
					...
					$count = count($this->collbotconfigs);
				}
			} else {
				$count = count($this->collbotconfigs);
			}
	...
	/**
	 * Method called to associate a botconfig object to this object
	 * through the botconfig foreign key attribute.
	 *
	 * @param      botconfig $l botconfig
	 * @return     void
	 * @throws     PropelException
	 */
	public function addbotconfig(botconfig $l)
	{
		if ($this->collbotconfigs === null) {
			$this->initbotconfigs();
		}
		if (!in_array($l, $this->collbotconfigs, true)) { // only add it if the **same** object is not already associated
			array_push($this->collbotconfigs, $l);
			$l->setsfGuardUser($this);
		}
	}
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related crosUserConfigs from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collcrosUserConfigs === null) {
			if ($this->isNew()) {
			   $this->collcrosUserConfigs = array();
				...
				$criteria->add(crosUserConfigPeer::USER_ID, $this->id);
	 ...
	 */
	public function getcrosUserConfigsJoinbotconfig($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collcrosUserConfigs === null) {
			if ($this->isNew()) {
				...
				$this->collcrosUserConfigs = crosUserConfigPeer::doSelectJoinbotconfig($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related licenses from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->colllicenses === null) {
			if ($this->isNew()) {
			   $this->colllicenses = array();
				...
				$criteria->add(licensePeer::USER_ID, $this->id);
	...
	public function countlicenses(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		...
		if ($this->colllicenses === null) {
			if ($this->isNew()) {
				...
				$count = licensePeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
	...
	/**
	 * Clears out the collbalanceOperations collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addbalanceOperations()
	 */
	public function clearbalanceOperations()
	{
		$this->collbalanceOperations = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collbalanceOperations collection (array).
	 *
	 * By default this just sets the collbalanceOperations collection to an empty array (like clearcollbalanceOperations());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initbalanceOperations()
	{
		$this->collbalanceOperations = array();
	}

	/**
	 * Gets an array of balanceOperation objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related balanceOperations from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array balanceOperation[]
	 * @throws     PropelException
	 */
	public function getbalanceOperations($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collbalanceOperations === null) {
			if ($this->isNew()) {
			   $this->collbalanceOperations = array();
				...
				$criteria->add(balanceOperationPeer::USER_ID, $this->id);

				balanceOperationPeer::addSelectColumns($criteria);
				$this->collbalanceOperations = balanceOperationPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				balanceOperationPeer::addSelectColumns($criteria);
				if (!isset($this->lastbalanceOperationCriteria) || !$this->lastbalanceOperationCriteria->equals($criteria)) {
					$this->collbalanceOperations = balanceOperationPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastbalanceOperationCriteria = $criteria;
		return $this->collbalanceOperations;
	}

	/**
	 * Returns the number of related balanceOperation objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related balanceOperation objects.
	 * @throws     PropelException
	 */
	public function countbalanceOperations(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collbalanceOperations === null) {
			if ($this->isNew()) {
				...
				$count = balanceOperationPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastbalanceOperationCriteria) || !$this->lastbalanceOperationCriteria->equals($criteria)) {
					...
					$count = count($this->collbalanceOperations);
				}
			} else {
				$count = count($this->collbalanceOperations);
			}
	...
	/**
	 * Method called to associate a balanceOperation object to this object
	 * through the balanceOperation foreign key attribute.
	 *
	 * @param      balanceOperation $l balanceOperation
	 * @return     void
	 * @throws     PropelException
	 */
	public function addbalanceOperation(balanceOperation $l)
	{
		if ($this->collbalanceOperations === null) {
			$this->initbalanceOperations();
		}
		if (!in_array($l, $this->collbalanceOperations, true)) { // only add it if the **same** object is not already associated
			array_push($this->collbalanceOperations, $l);
			$l->setsfGuardUser($this);
		}
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collbotSessions) {
				foreach ((array) $this->collbotSessions as $o) {
					$o->clearAllReferences($deep);
				}
			}
			if ($this->collbotconfigs) {
				foreach ((array) $this->collbotconfigs as $o) {
					$o->clearAllReferences($deep);
				}
			}
			...
			if ($this->collbalanceOperations) {
				foreach ((array) $this->collbalanceOperations as $o) {
					$o->clearAllReferences($deep);
				}
			}
